...
class PAGeRenViewController: PABaseViewController {
    ...
    lazy var gereView: PAGeRenView = {
        ...
        gereView.titleLabel.text = "Personal Information"
...
extension PAGeRenViewController {
    
    func GrePeonle() {
        ViewHud.addLoadView()
        let dict = ["affairs": self.productID ?? ""]
        PARequestManager.shared.requestAPI(params: dict, pageUrl: "/sicch/peopleLayer", method: .post) { [weak self] baseModel in
    ...
    func saveXinxi() {
        ...
        ViewHud.addLoadView()
        PARequestManager.shared.requestAPI(params: dict, pageUrl: "/sicch/thereFirstaidinstructions", method: .post) { [weak self] baseModel in
            let handsto = baseModel.handsto
            ...
            if handsto == 0 || handsto == 00 {
                self?.detailPageInfo(productID: self?.productID ?? "", startTime: self?.startime ?? "", type: "") { model1, model2, productID in
                    ...
                    self?.appmaidain(productID: self?.productID ?? "", type: "5", startTime: self?.startime ?? "")
                }
            }
            ViewHud.hideLoadView()
